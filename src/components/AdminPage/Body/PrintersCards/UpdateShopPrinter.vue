<template>
  <div style="background-color: #E0E0E0; height: 100%">
    <v-app-bar>
      <v-app-bar-title>
        <v-btn :to="`/admin/printers/${this.id_shop}/card`" class="btn btn-primary float-end m-3" color="primary"
               dark>
          Close
        </v-btn>
      </v-app-bar-title>
    ...
    <div class="my-form w-25" style="background-color: #8e8e96">
      <div class="card form-group ma-4">
        <div class="card-header">
          <h3 class=" float-end">Edit printer shop</h3>
        </div>
        <div class="card-body">
...
export default {
  ...
  methods: {
    getPlasticModelData(id) {
      let strUrl = "http://localhost:8080/shops/"
      axios.get(strUrl + id)
          .then(res => {
